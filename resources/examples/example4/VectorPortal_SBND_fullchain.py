"""
Full Dutta-Kim vector-portal chain at SBND (LAr)  —  MULTICHANNEL sum.
...
Adapted from DuttaKim_SBND_full_chain.py to:
  ...
  - target SBND argon (Ar40),
  - apply SBND cuts (volTPCActive sector + 140 MeV + 10deg) and sum observables.
...
R_LAR_INJECT = 4.5  # encloses the SBND active volume (corner ~3.81 m)
# Tight bounding box enclosing the two volTPCActive sectors:
_TPC_BOX_X = 4.10    # full width [m]  (two TPCs, +/-2.02 m)
_TPC_BOX_Y = 4.10    # full height [m] (+/-2.04 m)
_TPC_BOX_Z = 5.10   # full length [m]  (+/-2.51 m)
...
def make_meson_bias(sigma_fn=None):
    ...
    TO PHYSICAL RATE, focused forward where the boosted V1/chi can reach SBND.
...
# ------------------------------------------------------------------ #
#  Cut / observable helpers (SBND LAr, GDML volTPCActive sectors)    #
# ------------------------------------------------------------------ #
...
def run_channel(name, dk2nu_data, detector_model, n_events=events_to_inject, debug=False):
    ...
    #
    #   inject_box     — a tight axis-aligned Box that matches the actual SBND
    ...
    for ev, w in zip(events, raw):
        ...
        w_abs = (w * 2.0 * CALIB_VECTOR * SBND_POT
...
def main():
    ...
    ap.add_argument("--engine", choices=["analytic", "siren"], default="analytic",
                    help="'analytic' (default, AUTHORITATIVE sigma*N*chord cascade rate) or "
    ...
    print("\n" + "=" * 64)
    print("  MULTICHANNEL SUM  —  SBND Vector Portal (full chain)")
    print("=" * 64)
    ...
    os.makedirs("output", exist_ok=True)
    stem = "output/SBND_VectorPortal_multichannel_fullchain"
    ...
    #   cos theta (zoom): 0.8-1.0, 40 bins  (SBND/BNB forward-peak detail)
    ...
    # bins show the total expected signal events for the full SBND exposure.
    y_label = "Counts"
    ...
    ax[0].set_title(r"SBND $\chi$ upscattering: $E_\mathrm{vis}\ (e^+e^-)$ " + pot_note,
                    fontsize=10)
    ...
    ax[1].set_title(r"SBND $\chi$ upscattering: $\cos\theta$ wrt beam " + pot_note,
                    fontsize=10)
    ...
    ax[2].set_title(r"SBND $\chi$ upscattering: $\cos\theta$ zoomed $[0.80,1.0]$ " + pot_note,
                    fontsize=10)
