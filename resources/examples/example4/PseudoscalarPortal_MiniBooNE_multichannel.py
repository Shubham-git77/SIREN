"""
Long-lived PSEUDOSCALAR (a) Dark Primakoff chain at MiniBooNE (mineral oil)  —  MULTICHANNEL sum.
...
# ---- Pseudoscalar model (Table I/II of arXiv:2110.11944): m_a=1 MeV, m_Z'=85 MeV ----
# Table I best-fit (pseudoscalar): (m_Z', m_a) = (85, 1) MeV
#   g_mu * g_n * lambda = 5.9e-8 MeV^-1
# Table II individual benchmark (pseudoscalar):
#   g_mu = 1e-2,  g_n = 1e-2,  lambda = 6.5e-3 MeV^-1 = 6.5 GeV^-1
# NOTE: the SCALAR benchmark uses (m_Z'=49 MeV, g_mu=5e-3, lambda=0.44 GeV^-1) —
# all three differ from the pseudoscalar values above.
M_PHI = 1e-3                 # GeV  pseudoscalar mediator (a)
M_ZP  = 85e-3                # GeV  Z' dark Primakoff mediator (pseudoscalar: 85 MeV)
M_CARBON12 = 11.178          # GeV, C12 nuclear mass (MiniBooNE mineral oil)

# Couplings (Table II pseudoscalar benchmark)
G_MU_PROD = 1e-2             # a-muon coupling (g_mu);  scalar had 5e-3
G_N       = 1e-2             # Z'-nucleon coupling (g_n); same for both
LAMBDA    = 6.5              # GeV^-1 (= 6.5e-3 MeV^-1); scalar had 0.44 GeV^-1
...
def make_meson_bias(sigma_fn=None):
    ...
    bias = sigma(E_phi) * max(cos,0)   (candidate "C")
      ...
      that the forward-only shape over-sampled.  DO NOT reintroduce an E**n
      factor: sigma already carries the correct (decreasing) energy dependence.
...
def _build_sigma_interp(primakoff, e_lo=0.06, e_hi=8.0, n=120):
    ...
    sigma (~1e-30 cm^2) cannot push the sampling weights into precision loss."""
...
def run_channel(name, dk2nu_data, detector_model, n_events=events_to_inject, debug=False):
    ...
    # Primary interaction = meson -> l nu phi  (three-body pseudoscalar ME).
    ...
    for ev, w in zip(events, raw):
        ...
        w_abs = w * CALIB_SCALAR * MINIBOONE_POT * detection_efficiency(E_vis)
...
def main():
    ...
    print("\n" + "=" * 64)
    print("  MULTICHANNEL SUM  —  MiniBooNE Pseudoscalar Dark Primakoff")
    print("=" * 64)
    ...
    stem = "output/MiniBooNE_PseudoscalarPrimakoff_multichannel"
    ...
    ax[0].set_title(r"MiniBooNE pseudoscalar Dark Primakoff: $E_\gamma$ " + pot_note,
                    fontsize=10)
    ...
    ax[1].set_title(r"MiniBooNE pseudoscalar Dark Primakoff: $\cos\theta$ wrt beam " + pot_note,
                    fontsize=10)
    ...
    ax[2].set_title(r"MiniBooNE pseudoscalar Dark Primakoff: $\cos\theta$ zoom $[0.80,1.0]$ " + pot_note,
                    fontsize=10)
