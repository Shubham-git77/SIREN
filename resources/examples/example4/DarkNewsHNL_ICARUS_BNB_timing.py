r"""BNB dk2nu -> ICARUS DarkNews HNL timing (standalone ICARUS-only driver).

A single-detector variant of DarkNewsHNL_SBN_dk2nu_timing.py with the detector
pinned to ICARUS and the coordinate frame pinned to BNB, so it runs on a BNB
...
See DarkNewsHNL_SBN_dk2nu_timing.py and README_beam_timing.md for the details.
    ...
    dk2nu pi+ -> mu+ nu_mu -> N4 + Ar -> nu + gamma      (ICARUS, BNB frame)

Note: ICARUS sits ~600 m downstream of the BNB target and off the BNB axis,
so the BNB flux it sees is the small far/off-axis tail (ICARUS is primarily a
NuMI detector).  This is geometrically correct: the SBN detector model places
ICARUS at its surveyed position in the shared BNB/SAND geometry frame, so a
G4BNB file needs no coordinate transform.  Expect far fewer completed events
than the SBND driver at the same --events; raise --events accordingly.  Times
are nanoseconds relative to the primary proton.

Example::

    python DarkNewsHNL_ICARUS_BNB_timing.py                     # default BNB flux
    python DarkNewsHNL_ICARUS_BNB_timing.py /path/nubeam.dk2nu.root \
        --events 2000 --m4 0.10 --mu-tr-mu4 2.5e-6
"""
...
# This driver is ICARUS-only, BNB-only.
DETECTOR = "ICARUS"
BEAM_FRAME = "BNB"
