"""
Long-lived SCALAR (phi) Dark Primakoff chain at MiniBooNE (mineral oil)  —  MULTICHANNEL sum.
...
# Directed-channel acceptance correction -- DEBUNKED 2026-06-21, set to 1.0.
# The old 1/10.4 factor assumed SIREN's DetectorDirected channel over-counted the
# phi acceptance by 10.4x. That came from a phi||meson (collinear) estimate of the
# "physical" acceptance, which we showed OVER-counts the true decay-smeared
# acceptance by ~40x (measure_acceptance.py: phi||meson 5.2e-2 vs true 1.3e-3, the
# phi inherits a ~1/gamma_K~220 mrad emission cone). SIREN's REALIZED acceptance,
# extracted from the event tree (measure_siren_acceptance.py), is 8.5e-4 ~ the true
# 1.3e-3 -- i.e. the chain does NOT over-count, and importance sampling is unbiased,
# so NO correction is warranted. (For reliable absolute numbers use
# reliable_analytic_NS.py; the directed-mode sum here is high-variance/tail-prone.)
ACCEPTANCE_CORRECTION = 1.0
...
# MiniBooNE geometry: spherical mineral-oil tank.  The signal is selected in the
# fiducial sphere (R_FID); the phi Dark-Primakoff scatter is bounded to the full
# oil sphere (R_OIL).  Carbon (C12) + hydrogen are the oil scattering targets.
R_FID = 5.0          # fiducial sphere radius [m]
R_OIL = 5.746        # full mineral-oil tank radius [m]
...
# helicity-UNSUPPRESSED pi->e nu phi is large (for the pseudoscalar pi->e nu a it
# dominates, ~290x over the 320 excess). For a non-universal coupling, scale the
...
def make_meson_bias(sigma_fn=None):
    ...
    TO PHYSICAL RATE, focused forward where the boosted phi can reach the oil tank.
...
def build_onshell_models(parent_pdg, m_meson, m_lepton, lepton_pdg, nu_pdg):
    ...
    primakoff = _DP.DarkPrimakoffUpsCase(
        ...
        nuclear_pdgid=1000060120, nuclear_mass=M_CARBON12,
        A=12, Z=6, pdgid_phi=5919)                        # C12, phi->gamma
    return {
...
def primakoff_in_oil(event, oil_volume, debug=False):
    """Did a phi->gamma Primakoff scatter happen on C/H inside the oil sphere?"""
    try:
        for datum in event.tree:
            ...
            if 22 in secs and any(s in OIL_TARGET_PDGS for s in secs):
...
def signal_photon_observables(event, fid_volume):
...
def run_channel(name, dk2nu_data, detector_model, n_events=events_to_inject, debug=False):
    ...
    # Injection geometry — two concentric spheres (MiniBooNE oil tank):
    #   inject_sphere (R_OIL): bounds the phi Dark-Primakoff scatter vertex
    #   fiducial      (R_FID): directed channel target — aims phi into the oil.
    inject_sphere = siren.geometry.Sphere(R_OIL, 0.0)
    ...
    print("  Injection: oil sphere R=%.3fm  fiducial sphere R=%.3fm" %
          (R_OIL, R_FID))
    ...
    for ev, w in zip(events, raw):
        if not np.isfinite(w) or w <= 0:
            continue
        if not primakoff_in_oil(ev, oil_volume):
            continue
        obs = signal_photon_observables(ev, fid_volume)
        if obs is None:
            continue
        E_vis = obs[0]
        # Absolute normalization: C-R calibration × delivered BNB POT × efficiency.
        w_abs = (w * CALIB_SCALAR * ACCEPTANCE_CORRECTION
                 * MINIBOONE_POT * detection_efficiency(E_vis))
...
def main():
    ...
    if args.engine == "analytic":
        res = _AR.report_detector(sys.modules[__name__], "MiniBooNE scalar phi->gamma",
                                  "miniboone", vector=False, n_dec=args.n_dec)
        os.makedirs("output", exist_ok=True)
        _out = "output/MiniBooNE_scalar_analytic.npz"
    ...
    print("\n" + "=" * 64)
    print("  MULTICHANNEL SUM  —  MiniBooNE Scalar Dark Primakoff")
    print("=" * 64)
    ...
    os.makedirs("output", exist_ok=True)
    stem = "output/MiniBooNE_ScalarPrimakoff_multichannel"
    ...
    pot_note = r"(%.0e POT, MiniBooNE BNB)" % MINIBOONE_POT
    ...
    ax[0].set_title(r"MiniBooNE scalar Dark Primakoff: $E_\gamma$ " + pot_note,
                    fontsize=10)
    ...
    ax[1].set_title(r"MiniBooNE scalar Dark Primakoff: $\cos\theta$ wrt beam " + pot_note,
                    fontsize=10)
    ...
    ax[2].set_title(r"MiniBooNE scalar Dark Primakoff: $\cos\theta$ zoom $[0.80,1.0]$ " + pot_note,
                    fontsize=10)
