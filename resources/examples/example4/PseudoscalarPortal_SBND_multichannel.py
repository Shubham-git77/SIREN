"""
Long-lived PSEUDOSCALAR (a) Dark Primakoff chain at SBND (LAr)  —  MULTICHANNEL sum.
...
M_ARGON40  = 37.224          # GeV, Ar40 nuclear mass (SBND liquid argon)
...
# SBND LAr geometry (active-volume cut via GDML volTPCActive sectors).
# SBND has two TPCs (boxTPCActive 2.013 x 4.075 x 5.010 m each, at x=+/-4.5 mm),
# so the full active volume is ~4.04 x 4.075 x 5.01 m (farthest corner ~3.81 m).
# R_LAR_INJECT must exceed 3.81 m to enclose it.
R_LAR_INJECT = 4.5   # sphere radius [m] — encloses the SBND active volume (corner ~3.81 m)
_TPC_BOX_X   = 4.10   # full TPC x-extent [m]  (two TPCs, +/-2.02 m)
_TPC_BOX_Y   = 4.10   # full TPC y-extent [m]  (+/-2.04 m)
_TPC_BOX_Z   = 5.10   # full TPC z-extent [m]  (+/-2.51 m)
...
# SBND-specific curve when available; None -> efficiency 1.0 (placeholder).
_EFF_TABLE = None
...
def make_meson_bias(sigma_fn=None):
    ...
    TO PHYSICAL RATE, focused forward where the boosted phi can reach SBND.
...
def _in_lar_active(detector_model, vtx):
    """True if detector-frame point vtx is inside one of the 8 SBND
...
def run_channel(name, dk2nu_data, detector_model, n_events=events_to_inject, debug=False):
    ...
    for ev, w in zip(events, raw):
        ...
        w_abs = w * CALIB_SCALAR * SBND_POT * detection_efficiency(E_vis)
...
def main():
    ...
    print("\n" + "=" * 64)
    print("  MULTICHANNEL SUM  —  SBND Pseudoscalar Dark Primakoff")
    print("=" * 64)
    ...
    os.makedirs("output", exist_ok=True)
    stem = "output/SBND_PseudoscalarPrimakoff_multichannel"
    ...
    pot_note = r"(%.0e POT, SBND BNB)" % SBND_POT
    ...
    ax[0].set_title(r"SBND pseudoscalar Dark Primakoff: $E_\gamma$ " + pot_note,
                    fontsize=10)
    ...
    ax[1].set_title(r"SBND pseudoscalar Dark Primakoff: $\cos\theta$ wrt beam " + pot_note,
                    fontsize=10)
    ...
    ax[2].set_title(r"SBND pseudoscalar Dark Primakoff: $\cos\theta$ zoom $[0.80,1.0]$ " + pot_note,
                    fontsize=10)
