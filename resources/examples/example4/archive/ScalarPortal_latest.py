"""
Long-lived SCALAR (phi) Dark Primakoff chain at MiniBooNE  —  MULTICHANNEL sum.
...
def primakoff_in_oil(event, oil_volume):
    """Did a phi->gamma Primakoff scatter happen on C/H inside the oil?"""
    try:
...
def run_channel(name, dk2nu_data, detector_model, n_events=events_to_inject):
    ...
    # NOTE: do NOT call ResetInjectedEvents(n_events) here. The injector may
    # generate FAR fewer than n_events (acceptance-limited), and normalizing
    # the weight by the requested n_events instead of the actual generated
    # count makes the rate scale as 1/n_events (a normalization bug). We reset
    # to the ACTUAL generated count after the generation loop below.
    ...
    # ---- C++ weight-factor breakdown of the max-weight event ----
    # The Python FinalStateProbability methods are never called (the native
    # C++ Weighter computes the weight in the compiled layer), so the 1e9
    # weight spike must be isolated via the Weighter's own per-vertex
    # EventWeightBreakdown (interaction prob, position density, physical
    # probability, generation probability per vertex).
    def _dump_breakdown(_tag, _ev, _w):
        print("  [diag] BREAKDOWN %s (w=%.3e):" % (_tag, _w))
        # breakdown() takes an InteractionTree.  Try the common accessors.
        _tree = None
        for _attr in ("tree", "interaction_tree"):
            if hasattr(_ev, _attr):
                _tree = getattr(_ev, _attr)
                break
        if _tree is None:
            _tree = _ev  # last resort: the event may already be a tree
        try:
            _bd = weighter.breakdown(_tree)
        except Exception as _e:
            print("  [diag]   breakdown(tree) failed: %r" % _e)
            return
        # EventWeightBreakdown: introspect its fields.
        _fields = [a for a in dir(_bd) if not a.startswith("_")]
        print("  [diag]   fields: %s" % _fields)
        # Print scalar attributes directly; iterate per-vertex containers.
        for _f in _fields:
            try:
                _v = getattr(_bd, _f)
            except Exception:
            ...
            if callable(_v):
                continue
            print("  [diag]     %s = %r" % (_f, _v))

    try:
        _safe_raw = np.where(np.isfinite(raw) & (raw > 0), raw, -np.inf)
        if np.any(np.isfinite(_safe_raw) & (_safe_raw > 0)):
            _imax = int(np.nanargmax(_safe_raw))
            _dump_breakdown("MAX idx=%d" % _imax, events[_imax], raw[_imax])
            _pos = np.where(np.isfinite(raw) & (raw > 0))[0]
            if len(_pos):
                _med_idx = int(_pos[np.argsort(raw[_pos])[len(_pos) // 2]])
                _dump_breakdown("MEDIAN idx=%d" % _med_idx,
                                events[_med_idx], raw[_med_idx])
    except Exception as _e:
        print("  [diag] breakdown diagnostic failed: %r" % _e)
    # ---- end breakdown diagnostic ----
...
def main():
    ...
    ax[0].set_title("MiniBooNE Scalar Primakoff : photon energy"); ax[0].legend(fontsize=8)
    ax[1].set_xlabel("cos(theta) wrt beam"); ax[1].set_ylabel("Events / %g POT" % MINIBOONE_POT)
    ax[1].set_title("MiniBooNE Scalar Primakoff : photon angular"); ax[1].legend(fontsize=8)
