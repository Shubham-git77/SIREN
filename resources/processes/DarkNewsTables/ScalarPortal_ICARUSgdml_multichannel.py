...
def _in_lar_active(detector_model, vtx):
    """True if geometry-frame point vtx is inside one of the 8 ICARUS
    volTPCActive liquid-argon sectors (the GDML active volume)."""
    try:
        from siren.detector import GeometryPosition
        sec = detector_model.GetContainingSector(GeometryPosition(vtx))
        return (sec is not None) and sec.name.startswith("volTPCActive")
    except Exception:
        return False

def primakoff_in_lar(event, detector_model):
...
def main():
    ...
    ax[0].set_title("ICARUS Scalar Primakoff : photon energy"); ax[0].legend(fontsize=8)
    ax[1].set_xlabel("cos(theta) wrt beam"); ax[1].set_ylabel("Events / %g POT" % MINIBOONE_POT)
    ax[1].set_title("ICARUS Scalar Primakoff : photon angular"); ax[1].legend(fontsize=8)
