...
Weighter.save refuses configurations that would silently reload with different
physics (phase-space maps, Python-subclass models, stopping conditions),
matching Injector.save.
...
# ------------------------------------------------------------------ #
#  (b) save guard: configurations that reload with different physics  #
# ------------------------------------------------------------------ #

def test_save_refuses_phase_space_map_injector(tmp_path):
    """A raw injector whose process carries a phase-space map (not archived) is
    refused by Weighter.save; no file is written."""
    dm = detector.DetectorModel()
    xs = interactions.DummyCrossSection()
    int_col = interactions.InteractionCollection(_NuMu, [xs])
    p_inj = injection.PrimaryInjectionProcess()
    ...
    mc.weights = [1.0]
    p_inj.SetPhaseSpace(xs.GetPossibleSignatures()[0], mc)
    ...
    with pytest.raises(errors.NotSerializableError) as exc:
        weighter.save(base)
    assert any("phase space map" in o for o in exc.value.offenders)
...
# ------------------------------------------------------------------ #
#  (a) reloaded weights match the original (CCM-guarded)              #
# ------------------------------------------------------------------ #
