...
def test_run_requested_is_the_request_not_the_retry_budget():
    """A real Simulation.run(events=N) reports requested == N.

    Regression pin (not the _FakeInjector fixture) for the reporting
    corruption where the count leaked through the injector's live
    number_of_events: siren.Injector.generate raises the engine attempt quota
    to events * 1000, so a Results that snapshots number_of_events after
    generation reports requested == 1000 * N instead of N. Simulation.run now
    passes requested=events explicitly. This drives the cheapest genuine chain
    in the suite (IceCube + CC DIS splines, the TestSimulationRun pattern) so
    the real generate/weight path is exercised end to end.
    """
    N = 3
