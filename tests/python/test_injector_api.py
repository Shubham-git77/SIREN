...
def test_secondaries_without_expand_raises_at_build():
    ...
    validation now runs whenever a chain carries secondaries, so the
    misconfiguration raises a typed ConfigurationError instead.
    """
    from siren.vertex import Vertex
...
def test_pickle_refuses_phase_space_map():
    """__getstate__ runs the serialization guard, so a phase-space map is
    refused by pickle.dumps just as save() refuses it -- not silently dropped
    to reload with the wrong physics."""
    ...
    mc.channels = [injection.Isotropic2BodyChannel(0)]
    mc.weights = [1.0]
    inj.primary_phase_spaces = {sig: mc}   # attach to the built engine
    with pytest.raises(siren.errors.NotSerializableError) as exc:
        pickle.dumps(inj)
    assert any("phase space" in o for o in exc.value.offenders)
