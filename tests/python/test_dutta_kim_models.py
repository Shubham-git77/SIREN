...
def test_dark_sector_width_anchors(vector_portal):
    """Pin the corrected dark-sector partial widths to explicit spinor-sum
    ...
    # V1 -> ee shares the vector-pair implementation: the refactor reproduces
    # the (alpha eps^2 m_V/3) beta (1 + 2 r^2) inline form bit-for-bit.
...
def test_build_phi_flux_node_density_independent(meson_production_module):
    """build_phi_flux total is output-binning independent (the measure fix); the old node sum scaled with table density."""
    import numpy as np
