...
class Weighter:
    ...
    def save(self, filename: str):
        ...
        Refuses configurations that would silently reload with different
        physics. The engine archives processes but not their phase space maps,
        and it cannot serialize Python trampoline-derived interactions or
        distributions or an injector's stopping condition; any of these would
        reload with the fallback density (silently wrong weights), so they raise
        NotSerializableError instead.
    ...
    def _guard_serializable(self, _errors):
        ...
        Mirrors Injector._guard_serializable on the weighter's own physical
        side and defers to each injector's guard: a channel-based
        (phase-space-map) injector, a Python trampoline model, or a stopping
        condition would reload with the wrong physics, so ``save`` refuses them.
        """
        offenders = []
        for injector in (self.__injectors or []):
            if isinstance(injector, _PyInjector):
                ...
                processes = [injector.GetPrimaryProcess()] + list(
                    injector.GetSecondaryProcessMap().values())
                for process in processes:
                    if process.HasAnyPhaseSpace():
                        offenders.append(
                            "injector process {} carries a phase space map "
