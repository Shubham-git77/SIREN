...
class Injector:
    ...
    * the legacy keyword form (``number_of_events``, ``primary_type``,
    ...
    def save(self, filename):
        ...
        The engine archives processes (weighting mode included) but not their
        phase space map or a stopping condition, and it cannot serialize Python
        trampoline-derived interactions/distributions. Each would silently
        reload with the wrong physics, so they raise NotSerializableError
        instead. Pickle preserves the stopping condition, so pickle a chain
        that needs one.
        """
        from . import errors as _errors
        if self.__injector is None:
    ...
    def _guard_serializable(self, _errors, for_pickle=False):
        ...
        # pickle preserves it; only save() (the C++ archive) drops it.
