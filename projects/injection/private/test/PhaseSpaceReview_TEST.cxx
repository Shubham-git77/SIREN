...
// Reconstruct the removed overlap cap only to identify the part of a known
// lens that the legacy sampler could never reach.
...
TEST(DirectedOverlapSampling, CoversLensExcludedByLegacyCap) {
    ...
    // The legacy cap misses about 27% of this lens. A threshold well below
    // that makes this deterministic while still failing the old sampler.
...
TEST(DirectedRestSampling, BoundingSphereInteriorUsesFullSphereDensity) {
    ...
    // The old mismatch sampled these directions but assigned them zero
    // density. A full-sphere sampler should populate both hemispheres.
...
TEST(DirectedThresholdKinematics, CriticalCosineClampPreventsNaNRecord) {
    // This mass combination reproducibly exposes the cancellation from the
    // review: E_rest rounds just below m_daughter, so the unbounded critical-
    // cosine formula evaluates slightly above one at threshold.
...
TEST(PowerLawMapping, NuOneUsesNormalizedLogarithmicForm) {
    ...
    // Integrate in logarithmically spaced bins. The old nu=1 branch produced
    // a constant sample and NaN density instead of a normalized proposal.
    double integral = 0.0;
