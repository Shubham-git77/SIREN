...
void Injector::LoadInjector(std::string const & filename) {
    ...
    // The archive carries neither the RNG nor the stopping condition (Injector::save
    // omits both), so preserve the ones this injector already holds. The old in-place
    // load never touched them; the temp-and-move below would otherwise overwrite them
    // with the temporary's defaults -- and a null RNG segfaults the next GenerateEvent.
