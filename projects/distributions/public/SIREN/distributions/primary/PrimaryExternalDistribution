...
class PrimaryExternalDistribution :
    virtual public PrimaryInjectionDistribution,
    virtual public VertexPositionDistribution {
...
private:
    ...
    // Mutable cache: set by Sample(), consumed by SamplePosition().
    // Allows SamplePosition() to return a non-degenerate segment centred on
    // the CSV decay point so the injector can compute direction and length.
    mutable std::array<double, 3> _cached_position  = {0.0, 0.0, 0.0};
    mutable std::array<double, 3> _cached_direction = {0.0, 0.0, 1.0};
...
public:
    ...
    void Sample(std::shared_ptr<siren::utilities::SIREN_random> rand,
    ...
    // VertexPositionDistribution pure-virtual implementations.
    // SamplePosition returns a tiny segment centred on the CSV decay point
    // so that the injector obtains a valid direction and non-zero length.
    ...
    virtual std::tuple<siren::math::Vector3D, siren::math::Vector3D>
        InjectionBounds(std::shared_ptr<siren::detector::DetectorModel const> detector_model,
                        std::shared_ptr<siren::interactions::InteractionCollection const> interactions,
                        siren::dataclasses::InteractionRecord const & record) const override;

    virtual double GenerationProbability(std::shared_ptr<siren::detector::DetectorModel const> detector_model,
                                         std::shared_ptr<siren::interactions::InteractionCollection const> interactions,
                                         siren::dataclasses::InteractionRecord const & record) const override;
